...
const ls = document.getElementById('ls2');
ls.addEventListener('change', ()=>{
    document.body.classList.toggle('low-stimulus', ls.checked);
    applyLowStimulus();
});

// Xử lý form đăng nhập (demo)
const form = document.getElementById('loginForm');
const err = document.getElementById('error');
form.addEventListener('submit', (e)=>{
    err.style.display='none'; err.textContent='';
    const email = form.email.value.trim(), pw = form.password.value.trim();
    if(!email || !pw){
        e.preventDefault();
        err.textContent = 'Vui lòng điền đầy đủ thông tin. Không sao cả—chúng ta làm lại nhé.';
        err.style.display='block'; return;
    }
    if (pw.length < 6) {
        e.preventDefault();
        err.textContent = 'Mật khẩu phải ít nhất 6 ký tự.';
        err.style.display='block'; return;
    }
    e.preventDefault();
    alert('Đăng nhập (mẫu). Kết nối API backend để xác thực.');
